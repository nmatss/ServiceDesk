...
/**
 * Registra uma operacao de IA no audit trail
 */
export async function logOperation(
  ...
  // Calcular data de expiracao baseada no tipo de operacao (LGPD compliance)
  const retentionPeriodMonths = getRetentionPeriod(operationType);
  ...
  const auditEntry = {
    ...
    processing_legal_basis: determineLegalBasis(operationType),
    data_retention_expires_at: expirationDate.toISOString(),
    consent_given: true // Assumindo consentimento para operacoes do ServiceDesk
  };
...
/**
 * Registra feedback do usuario sobre uma operacao de IA
 */
export async function logFeedback(
  ...
  if (wasCorrect === false || rating && rating <= 2) {
    await createTrainingDataFromFeedback(auditId, feedback);
  }
}
 ...
 */
export async function queryAuditTrail(query: AIAuditQuery): Promise<AIAuditEntry[]> {
  let sql = `
...
/**
 * Obtem estatisticas do audit trail
 */
export async function getAuditStats(
  ...
  // Metricas de performance - dialect-aware JSON extraction
  const jsonExtract = getDatabaseType() === 'postgresql'
  ...
  const performanceStats = await executeQueryOne<any>(`
    SELECT
      ...
      SUM(${jsonExtract('metadata', 'outputTokens')}) as total_output_tokens,
      SUM(${jsonExtract('metadata', 'estimatedCost')}) as total_estimated_cost
    FROM ai_audit_logs
    ${whereClause}
  `, params);

  // Feedback e satisfacao - dialect-aware JSON extraction
  const feedbackExtractRating = getDatabaseType() === 'postgresql'
    ? `(user_feedback::json->>'rating')::numeric`
    : `JSON_EXTRACT(user_feedback, '$.rating')`;

  const feedbackExtractHelpful = getDatabaseType() === 'postgresql'
  ...
  const complianceIssues = await executeQueryOne<any>(`
    ...
    FROM ai_audit_logs
    ${whereClause}
      AND (consent_given = false OR data_retention_expires_at < ${nowExpr})
  `, params);
  ...
  const totalOps = basicStats?.total_operations || 1;

  return {
    totalOperations: basicStats?.total_operations || 0,
...
export async function cleanupExpiredData(): Promise<number> {
  ...
  const result = await executeRun(`
    ...
    WHERE data_retention_expires_at < ${nowExpr}
  `);
...
/**
 * Obtem operacoes com problemas de compliance
 */
export async function getComplianceIssues(): Promise<AIAuditEntry[]> {
  return await queryAuditTrail({
...
/**
 * Gera relatorio de uso de IA para auditoria
 */
export async function generateAuditReport(
  ...
  const summary = await getAuditStats(periodStart, periodEnd, organizationId);
  const operationDetails = await queryAuditTrail({
    organizationId,
  ...
  const complianceIssuesList: string[] = [];
  const recommendations: string[] = [];

  if (summary.complianceIssues > 0) {
    complianceIssuesList.push(`${summary.complianceIssues} operacoes com problemas de retencao de dados`);
    recommendations.push('Revisar politicas de retencao de dados');
  }

  if (summary.feedbackRate < 0.1) {
    complianceIssuesList.push('Taxa de feedback muito baixa para validacao de qualidade');
    recommendations.push('Implementar coleta automatica de feedback');
  }

  if (summary.successRate < 0.9) {
    complianceIssuesList.push('Taxa de sucesso abaixo do esperado');
  ...
  return {
    ...
    operationDetails,
    complianceStatus: {
      isCompliant: complianceIssuesList.length === 0,
...
/**
 * Determina o periodo de retencao baseado no tipo de operacao
 */
function getRetentionPeriod(operationType: AIOperationType): number {
  const retentionPeriods: Record<AIOperationType, number> = {
    classification: 24, // 2 anos
    suggestion: 12, // 1 ano
    response_generation: 6, // 6 meses
    sentiment_analysis: 12, // 1 ano (dados sensiveis)
 ...
 */
function determineLegalBasis(operationType: AIOperationType): string {
 ...
 */
async function createTrainingDataFromFeedback(
  auditId: number,
  feedback: any
): Promise<void> {
  try {
...
// Criar tabela de audit logs se nao existir
export async function createAIAuditTable(): Promise<void> {
  if (getDatabaseType() === 'sqlite') {
    ...
    await executeRun(`CREATE INDEX IF NOT EXISTS idx_ai_audit_logs_operation_type ON ai_audit_logs(operation_type)`);
    await executeRun(`CREATE INDEX IF NOT EXISTS idx_ai_audit_logs_entity ON ai_audit_logs(entity_type, entity_id)`);
    await executeRun(`CREATE INDEX IF NOT EXISTS idx_ai_audit_logs_user ON ai_audit_logs(user_id)`);
    await executeRun(`CREATE INDEX IF NOT EXISTS idx_ai_audit_logs_organization ON ai_audit_logs(organization_id)`);
    await executeRun(`CREATE INDEX IF NOT EXISTS idx_ai_audit_logs_created ON ai_audit_logs(created_at)`);
    await executeRun(`CREATE INDEX IF NOT EXISTS idx_ai_audit_logs_retention ON ai_audit_logs(data_retention_expires_at)`);
    await executeRun(`CREATE INDEX IF NOT EXISTS idx_ai_audit_logs_operation_id ON ai_audit_logs(operation_id)`);
  }
