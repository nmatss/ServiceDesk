  ...
  return (
    <div className={`p-3 sm:p-4 rounded-lg border ${colorClasses[color]}`}>
      <div className="flex items-start justify-between mb-1 sm:mb-2">
        ...
        {trend !== undefined && (
          <div className={`flex items-center gap-0.5 flex-shrink-0 ${trend > 0 ? 'text-green-600' : trend < 0 ? 'text-red-600' : 'text-neutral-500'}`}>
      ...
      <div className="flex items-baseline gap-1">
        ...
        <span className="text-xs sm:text-sm text-neutral-500">{unit}</span>
      </div>
      {description && (
        <p className="text-[10px] sm:text-xs text-neutral-500 mt-1 line-clamp-2">{description}</p>
      )}
    </div>
  )
}
    ...
    <div className="bg-white dark:bg-neutral-800 rounded-xl shadow-sm dark:shadow-neutral-900/20 border border-neutral-200 dark:border-neutral-700 p-4 sm:p-6">
      ...
      <div className="flex items-center gap-4 sm:gap-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs sm:text-sm text-neutral-600 line-clamp-3 sm:line-clamp-none">{description}</p>
        </div>
      </div>
    </div>
  )
